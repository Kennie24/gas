...
const SocialTooltip = React.forwardRef<HTMLUListElement, SocialTooltipProps>(
  ({ className, items, ...props }, ref) => {
    ...
    const baseTooltipStyles =
      "absolute left-1/2 top-[-36px] z-[100] -translate-x-1/2 px-2.5 py-1.5 text-sm text-white whitespace-nowrap rounded-md opacity-0 invisible transition-all duration-300 ease-in-out group-hover:opacity-100 group-hover:visible group-hover:top-[-44px]";
        ...
        {items.map((item) => (
          <li key={item.ariaLabel} className="relative group">
            ...
            >
              ...
              />
              {item.icon ? (
                <span className={cn(baseLineIconStyles)} aria-hidden="true">
                  {item.icon}
                </span>
              ) : item.svgUrl ? (
                <img
